...
def get(message):
    if message.from_user.id != 779636559:
        return
    ...
    if text == '/get logs':
        try:
            ...
            bot.send_message(message.chat.id, 'No logs', parse_mode='html')
    elif text == '/get json':
        try:
            file = open(jsonfilename, 'r')
            bot.send_document(message.chat.id, file, parse_mode='html')
        except:
            bot.send_message(message.chat.id, 'No logs', parse_mode='html')
    elif text == '/get chats':
        try:
            ...
            bot.send_message(message.chat.id, 'No chats', parse_mode='html')
